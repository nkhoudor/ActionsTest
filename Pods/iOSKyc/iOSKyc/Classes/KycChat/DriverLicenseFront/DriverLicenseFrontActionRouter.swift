...
class DriverLicenseFrontActionRouter : PhotoActionRouter {
    ...
    override func photoAction() {
        ...
        router.imageTaken = { [weak self] photoData in
            ...
            self?.swipeVC = nil
            self?.viewModel.documentType = .driverLicense
            self?.viewModel.frontImageFile = photoData
            self?.chatFlowService.state.accept(.DRIVER_LICENSE_FRONT_TAKEN(photoData: photoData))
        }
        
        let vc = MaskCameraVC.createInstance(presenter: MaskCameraPresenter(interactor: MaskCameraInteractor(), router: router, configurator: DriverLicenseFrontMaskCameraConfigurator()))
