//
//  DriverLicenseBackActionRouter.swift
//  iOSKyc
//
...
class DriverLicenseBackActionRouter : PhotoActionRouter {
    ...
    override func photoAction() {
        ...
        router.imageTaken = { [weak self] photoData in
            ...
            self?.chatFlowService.state.accept(.DRIVER_LICENSE_BACK_TAKEN(photoData: photoData))
        }
        
        let vc = MaskCameraVC.createInstance(presenter: MaskCameraPresenter(interactor: MaskCameraInteractor(), router: router, configurator: DriverLicenseBackMaskCameraConfigurator()))
